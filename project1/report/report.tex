...
\caption{Training BPR loss per epoch. MF drives the loss down faster and
further (its curves stop early because of early stopping), yet ranks held-out
items worse.}
\label{fig:loss}
\end{figure}

The training-loss curves (Figure~\ref{fig:loss}) complete the picture from
the optimization side: MF fits the training pairs faster and more aggressively
than LightGCN, yet generalizes worse (Table~\ref{tab:main}). The propagation
step acts as a structural regularizer --- each embedding is tied to its
neighborhood average, so LightGCN cannot minimize the loss by memorizing
individual pairs. The gap between training fit and test ranking is precisely
the graph prior at work, and it also explains the slower convergence observed
above.
